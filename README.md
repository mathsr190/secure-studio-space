...
## スタジオ管理者の利用
スタジオ管理者としてログイン。
スタジオ管理者はマイページから登録したスタジオの確認ができます。
スタジオの詳細ページにスタジオ情報編集ボタンが現れます。そこから編集が可能です。
各部屋のページに情報編集ボタンが現れます。そこから編集が可能です。
スタジオ・部屋ともに削除はできない仕様となっています。

## 利用者の利用
利用者としてログインします。
スタジオの各ページの詳細情報が見られます。
各部屋のページに予約ボタンが現れます。そこから予約が可能です。
予約情報はマイページから確認ができます。

# 目指した課題解決(制作背景)
音楽活動をする上で重要なのが練習場所の確保です。しかし、練習場所となるそれぞれのスタジオが各ページを所有して、見た目もバラバラ、掲載情報もバラバラなのが現状です。利用者にとっては情報を得るのが煩わしく、またスタジオ管理者にとっては新規利用見込みの方にアプローチしづらいといった課題があります。統一したページにすることで、それらの課題を解決できると考えました。
...
deviseを用いて実装しました。ログイン後はトップページに遷移し、ログイン中のスタジオ名が表示されます。  
※本来アプリケーションとしては利用者ログインと同じ画面に並んではいけませんが、実装した機能を分かりやすくするためにこのような表示となっています。
...
studio登録、編集ともに同じフォームを利用しています。managerログイン時のみ編集できます。
...
space登録、編集ともに同じフォームを利用しています。managerログイン時のみ編集できます。
...
deviseを用いて実装しました。ログイン後はトップページに遷移し、ログイン中のuser名が表示されます。  
※本来アプリケーションとしては利用者ログインと同じ画面に並んではいけませんが、実装した機能を分かりやすくするためにこのような表示となっています。
...
予約をするとマイページに予約情報が表示されます。  
※料金を入力できる仕様になっています。本来は自動計算して表示させるべきであるが、予約機能そのものを優先して実装しました。
...
# 工夫した点
- レイアウトが整った状態で、見た目から得られる情報をなるべくシンプルにしました。
- CSSの編集などが後から編集しやすいように、ファイルの入れる場所を整理しました。